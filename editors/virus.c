...
char *vi_Version = "0.0.2+crlserver";

/*
 * To compile:
 *	gcc -Wall -Os -s -o virus virus.c
 *	strip virus
 */
      ...
      for (i = 0; i < cnt; i++)
        {
          if (thing == 0)
            ...
            {                   // insert words
              strlcat ((char *) readbuffer, words[(int) lrand48 () % 20], BUFSIZ);
              strlcat ((char *) readbuffer, " ", BUFSIZ);
              sleeptime = 0;    // how fast to type
            }
          else if (thing == 2)
            {                   // insert lines
              strlcat ((char *) readbuffer, lines[(int) lrand48 () % 20], BUFSIZ);
              sleeptime = 0;    // how fast to type
            }
          else
            {                   // insert multi-lines
              strlcat ((char *) readbuffer, multilines[(int) lrand48 () % 20], BUFSIZ);
              sleeptime = 0;    // how fast to type
            }
        }
...
crash_test ()
{
  ...
  if (end < text)
    {
      strlcat ((char *) msg, "end<text ", sizeof(msg));
    }
  if (end > textend)
    {
      strlcat ((char *) msg, "end>textend ", sizeof(msg));
    }
  if (dot < text)
    {
      strlcat ((char *) msg, "dot<text ", sizeof(msg));
    }
  if (dot > end)
    {
      strlcat ((char *) msg, "dot>end ", sizeof(msg));
    }
  if (screenbegin < text)
    {
      strlcat ((char *) msg, "screenbegin<text ", sizeof(msg));
    }
  if (screenbegin > end - 1)
    {
      strlcat ((char *) msg, "screenbegin>end-1 ", sizeof(msg));
    }
  ...
  if (tim >= (oldtim + 3))
    {
      snprintf ((char *) status_buffer, BUFSIZ_STATBUF,
      ...
      if (c1 != 27)
        {
          ...
          if (c == 'c')
            {
              strlcpy ((char *) buf, "Change", sizeof(buf));
              goto dc_i;        // start inserting
            }
          if (c == 'd')
            {
              strlcpy ((char *) buf, "Delete", sizeof(buf));
            }
#ifdef BB_FEATURE_VI_YANKMARK
          if (c == 'y' || c == 'Y')
            {
              strlcpy ((char *) buf, "Yank", sizeof(buf));
            }
          p = reg[YDreg];
          q = p + strlen ((char *) p);
          for (cnt = 0; p <= q; p++)
...
colon (Byte * buf)
{
  ...
  */
  ...
  while (isblnk (*buf))
    buf++;
  strlcpy ((char *) args, (char *) buf, BUFSIZ);
...
char_insert (Byte * p, Byte c)  // insert the char c at 'p'
{
  if (c == 22)
    ...
    {                           // Is this an ESC?
      ...
      cmdcnt = 0;
      end_cmd_q ();             // stop adding to q
      strlcpy ((char *) status_buffer, " ", BUFSIZ_STATBUF); // clear the status buffer
      //if ((p[-1] != '\n') && (dot > text))
      //  {
          p--;
      //  }
...
print_literal (Byte * buf, Byte * s)  // copy s to buf, convert unprintable
{
  ...
  b[1] = '\0';
  strlcpy ((char *) buf, "", sizeof(buf));    // init buf
  if (strlen ((char *) s) <= 0)
    s = (Byte *) "(NULL)";
  for (; *s > '\0'; s++)
    {
      ...
      if (*s > '~')
        {
          strlcat ((char *) buf, SOs, sizeof(buf));
          c = *s - 128;
        }
      if (*s < ' ')
        {
      ...
      b[0] = c;
      strlcat ((char *) buf, (char *) b, sizeof(buf));
      if (*s > '~')
        strlcat ((char *) buf, SOn, sizeof(buf));
      if (*s == '\n')
        {
          strlcat ((char *) buf, "$", sizeof(buf));
        }
...
start_new_cmd_q (Byte c)
{
  ...
  if (cmdcnt > 0)
    snprintf ((char *) last_modifying_cmd, BUFSIZ, "%d", cmdcnt);
...
get_input_line (Byte * prompt)  // get input line- use "status line"
{
  ...
  if (strlcpy((char *) buf, (char *) prompt, sizeof((char *)buf)) > sizeof ((char *) buf))
...
place_cursor (int row, int col, int opti)
{
  ...
  snprintf ((char *) cm1, BUFSIZ, CMrc, row + 1, col + 1);
  cm = cm1;
  if (opti == FALSE)
    goto pc0;
  ...
  while (row < Rrow)
    {
      ...
      strlcat (cm2, CMup, BUFSIZ);
      Rrow--;
    }
  while (row > Rrow)
    {
      // the cursor has to move down
      strlcat (cm2, CMdown, BUFSIZ);
      Rrow++;
    }

  // now move to the correct column
  strlcat (cm2, "\r", BUFSIZ);           // start at col 0
